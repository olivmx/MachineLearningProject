...
#this code will now show is just to load needed libraries.
setwd("C:\\TestR\\JHS_Dev_Data_Products\\project")
library(caret);library(ggplot2);library(gbm)
```

The following R commands are to download training and test set for the study.
````{r cache=TRUE, results='hide'}
rawTraining <- read.table("C:\\TestR\\JHS_Machine_Learning\\project\\pml-training.csv", sep=",", header=T)
rawTesting <- read.table("C:\\TestR\\JHS_Machine_Learning\\project\\pml-testing.csv", sep=",", header=T)
````
...
This commands will fit the **Boosting** model with the following parameters:  
1. Method: "gbm"  
...
The principal component analysis is set to measure up to 90% of variations to avoid overfitting of the training set. A total of 19 components are nedded for this accuracy.
